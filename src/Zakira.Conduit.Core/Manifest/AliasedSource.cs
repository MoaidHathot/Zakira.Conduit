...
/// <summary>
///     A transient source wrapper that pairs an inner <see cref="ISource"/>
...
///         fetcher: the <see cref="Sources.Inference.SourceInferenceCoordinator"/>
///         unwraps every <see cref="AliasedSource"/> at manifest-load
///         time, applies <see cref="As"/> to the owning entry's name (or to
///         the synthesized name of an array-expanded sub-entry), and then
///         drops the wrapper.
///     </para>
///     <para>
///         <see cref="AliasedSource"/> is intentionally <b>not</b>
///         registered as a <c>[JsonDerivedType]</c> on
///         <see cref="ISource"/>: it has no <c>type</c> discriminator and
///         is recognised exclusively by the shorthand converter (which peeks
///         for the wrapper shape <c>{ source, as }</c>).
///     </para>
/// </remarks>
public sealed record AliasedSource : ISource
{
    /// <summary>
    ...
    /// <summary>The wrapped source. Required, never itself an <see cref="AliasedSource"/>.</summary>
    public required ISource Inner { get; init; }
