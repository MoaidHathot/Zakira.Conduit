...
/// <summary>
///     <see cref="ISourceFetcher"/> for <see cref="LocalDirectorySource"/>.
///     <para>
...
///     </para>
/// </summary>
public sealed class LocalDirectorySourceFetcher : ISourceFetcher
{
    private readonly IPathResolver _pathResolver;
    private readonly ILogger<LocalDirectorySourceFetcher> _logger;

    public LocalDirectorySourceFetcher(IPathResolver pathResolver, ILogger<LocalDirectorySourceFetcher> logger)
    {
    ...
    /// <inheritdoc />
    public string SourceKind => LocalDirectorySource.TypeDiscriminator;
    ...
    public Task<FetchedSource> FetchAsync(ISource source, FetchContext context, CancellationToken cancellationToken = default)
    {
        ...
        if (source is not LocalDirectorySource local)
        {
            throw new ArgumentException($"Expected a {nameof(LocalDirectorySource)} but got '{source.GetType().Name}'.", nameof(source));
        }
