...
/// <summary>
///     <see cref="ISourceFetcher"/> for <see cref="AzdoSource"/>.
...
public sealed class AzdoSourceFetcher : ISourceFetcher
{
    ...
    /// <inheritdoc />
    public string SourceKind => AzdoSource.TypeDiscriminator;
    ...
    public async Task<FetchedSource> FetchAsync(ISource source, FetchContext context, CancellationToken cancellationToken = default)
    {
        ...
        if (source is not AzdoSource azdo)
        {
            throw new ArgumentException($"Expected an {nameof(AzdoSource)} but got '{source.GetType().Name}'.", nameof(source));
        }
    ...
    private async Task DownloadAndExtractAsync(AzdoSource source, string commitSha, string? scopePath, string destinationDir, CancellationToken cancellationToken)
    {
