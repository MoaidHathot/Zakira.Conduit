...
public sealed class GitHubSourceFetcherTests
{
    ...
    public async Task End_to_end_fetch_extracts_repository_content_locally()
    {
        ...
        var source = new GitHubSource { Repo = "anthropics/skills", Path = "skills/code-review", Branch = "main" };
    ...
    public async Task Disposing_fetched_source_removes_temp_directory()
    {
        ...
        var fetcher = BuildFetcher(server);
        var source = new GitHubSource { Repo = "o/r" };
    ...
    public async Task Subpath_that_does_not_exist_throws()
    {
        ...
        var fetcher = BuildFetcher(server);
        var source = new GitHubSource { Repo = "o/r", Path = "does/not/exist" };
    ...
    public async Task Uses_commit_ref_when_set()
    {
        ...
        var fetcher = BuildFetcher(server);
        var source = new GitHubSource { Repo = "o/r", Commit = "abc123" };
    ...
    public async Task Multi_path_fetch_makes_a_single_http_request()
    {
        ...
        var fetcher = BuildFetcher(server);
        var source = new GitHubSource { Repo = "acme/skills", Paths = ["a", "b", "c"] };
    ...
    public async Task Multi_path_fetch_fails_fast_when_one_path_is_missing()
    {
        ...
        var fetcher = BuildFetcher(server);
        var source = new GitHubSource { Repo = "acme/skills", Paths = ["good", "missing"] };

        var act = () => fetcher.FetchAsync(source, DefaultContext);
        await act.Should().ThrowAsync<GitHubDownloadException>();
    }

    private static ISourceFetcher BuildFetcher(MockGitHubServer server)
    {
        ...
        var registry = provider.GetRequiredService<ISourceFetcherRegistry>();
        return registry.GetFetcher(new GitHubSource { Repo = "x/y" });
    }
}
