...
public sealed class LocalDirectorySourceFetcherTests
{
    [Fact]
    public async Task Returns_a_fetched_source_pointing_at_the_local_directory()
    {
        ...
        var source = new LocalDirectorySource { Path = sourceDir };
    ...
    public async Task Relative_path_is_resolved_against_manifest_directory()
    {
        ...
        var fetcher = BuildFetcher();
        var context = new FetchContext(tmp.Combine("conduit.json"));
        var source = new LocalDirectorySource { Path = "./skills/alpha" };
    ...
    public async Task Tilde_is_expanded_to_the_home_directory()
    {
        ...
        var fetcher = new LocalDirectorySourceFetcher(resolver, NullLogger<LocalDirectorySourceFetcher>.Instance);

        var context = new FetchContext(tmp.Combine("conduit.json"));
        var source = new LocalDirectorySource { Path = "~/skills" };
    ...
    public async Task Missing_directory_throws_LocalSourceNotFound()
    {
        ...
        var fetcher = BuildFetcher();
        var context = new FetchContext(tmp.Combine("conduit.json"));
        var source = new LocalDirectorySource { Path = tmp.Combine("does-not-exist") };
    ...
    public async Task Wrong_source_type_throws()
    {
        ...
        var act = () => fetcher.FetchAsync(new GitHubSource { Repo = "o/r" }, context);

        await act.Should().ThrowAsync<ArgumentException>();
    }

    private static LocalDirectorySourceFetcher BuildFetcher()
    {
        var env = new FakeEnvironment
        {
        ...
        return new LocalDirectorySourceFetcher(new DefaultPathResolver(env), NullLogger<LocalDirectorySourceFetcher>.Instance);
    }
}
